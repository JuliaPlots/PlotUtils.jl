...
plot_color(s::Symbol) = haskey(color_libraries[_gradients[1]],s) ? cgrad(s) : parse(RGBA{Float64}, s)
...
plot_color(s::Symbol, α::Number) = (haskey(color_libraries[_gradients[1]],s) ? cgrad(s, alpha=α) : RGBA{Float64}(convert(RGB, plot_color(s)), α))
